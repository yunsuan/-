...
@interface YBImageBrowser () <YBImageBrowserViewDelegate, YBImageBrowserViewDataSource, YBImageBrowserToolBarDelegate, YBImageBrowserFunctionBarDelegate, UIViewControllerTransitioningDelegate,XGToolBarDelegate> {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!isDealViewDidAppear) {
        ...
        [self.view addSubview:self.xgToolBar];
...
- (void)XGBarScrollToIndex:(NSInteger)index{
    ...
    [self.browserView scrollToPageWithIndex:(index - 1)];
//    [self setTooBarNumberCountWithCurrentIndex:index];
}
