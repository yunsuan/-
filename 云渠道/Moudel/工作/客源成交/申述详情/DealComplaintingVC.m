...
-(void)initDataSouce
{
    ...
    _titleArr = @[@"推荐编号",@"处理信息",@"申诉信息",@"无效信息",@"到访确认信信息"];
    ...
    if (indexPath.section == 4 && indexPath.row > 2) {
        ...
        if (indexPath.section == 0) {
            ...
            if (indexPath.row == 0) {
                ...
                [cell SetCellContentbystring:[NSString stringWithFormat:@"无效描述：%@",_dataDic[@"disabled_reason"]]];
            }else{
                
                [cell SetCellContentbystring:[NSString stringWithFormat:@"无效时间：%@",_dataDic[@"recommend_time"]]];
            }
